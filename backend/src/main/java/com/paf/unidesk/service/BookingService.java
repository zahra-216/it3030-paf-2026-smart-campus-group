...
public class BookingService {
    ...
    public Booking createBooking(Booking booking) {
        ...
        if (!conflicts.isEmpty()) {
            ...
            throw new RuntimeException(
                    "Resource is INACTIVE from " + conflict.getStartTime() +
                    " to " + conflict.getEndTime() + ". It becomes ACTIVE after that time."
            );
        }
    ...
    public String getResourceStatusForTime(Booking booking) {
        ...
        if (!conflicts.isEmpty()) {
            Booking conflict = conflicts.get(0);
            return "INACTIVE from " + conflict.getStartTime() + " to " +
                    conflict.getEndTime() + ", ACTIVE after that";
        }
        return "ACTIVE";
    }
