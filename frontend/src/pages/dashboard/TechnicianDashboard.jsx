...
export default function TechnicianDashboard() {
    ...
    // ── status modal state ─────────────────────────────────────────────────
    // pendingStatus is STRICTLY either "IN_PROGRESS" or "RESOLVED"
    // The modal only ever opens for one of these — never skips a step
    ...
    // ── derived stats ──────────────────────────────────────────────────────
    const open       = myTickets.filter(t => normStatus(t.status) === "OPEN").length;
    const inProgress = myTickets.filter(t => normStatus(t.status) === "IN_PROGRESS").length;
    const high       = myTickets.filter(t => normPriority(t.priority) === "HIGH").length;
    ...
    //
    //  Rule:  OPEN → IN_PROGRESS only
    //         IN_PROGRESS → RESOLVED only
    //  No other transitions are permitted from here.
    //
    const openStatusModal = (ticket, targetStatus) => {
        const current = normStatus(ticket.status);

        // Hard guards — silently reject invalid transitions
    ...
    const confirmStatusChange = async () => {
        ...
        try {
            ...
            // Also refresh the detail modal if it's open for the same ticket
    ...
    //
    //  OPEN        →  blue  "▶ Start"  button
    //  IN_PROGRESS →  green "✓ Mark Resolved" button
    //  RESOLVED    →  "✓ Resolved" text label (no further action)
    //  CLOSED / REJECTED / other → status text only
    //
    ...
    return (
        <div style={styles.page}>
            ...
            {high > 0 && (
                <div style={styles.alert}>
                    ...
                    <p style={styles.alertText}>
                        You have <strong>{high} high priority</strong> ticket{high > 1 ? "s" : ""} that need{high === 1 ? "s" : ""} immediate attention.
                    </p>
                </div>
            )}
                            ...
                            {myTickets.map(t => {
                                ...
                                return (
                                    <tr key={t.id} className="tkrow"
                                        ...
                                        {/* Stop row-click so button click doesn't open detail modal */}
            ...
            {/* ══════════════════════════════════════════════════════════════════
                STATUS CHANGE MODAL
                Opens for either  OPEN→IN_PROGRESS  or  IN_PROGRESS→RESOLVED
                Never skips a step.
            ══════════════════════════════════════════════════════════════════ */}
                ...
                <div style={overlayStyle} onClick={e => e.target === e.currentTarget && setShowStatusModal(false)}>
                    <div style={modalBox}>
                        ...
                        <div style={{ padding: "1.375rem 1.625rem", display: "flex", flexDirection: "column", gap: 16 }}>
                            ...
                            {/* ── OPEN → IN_PROGRESS: just a confirmation ── */}
                        ...
                        <div style={modalBoxWide}>
                            ...
                            <div style={{ padding: "1.375rem 1.625rem", display: "flex", flexDirection: "column", gap: 16 }}>
                                {detailLoading ? (
                                    ...
                                    {/* next-step action banner — only for OPEN or IN_PROGRESS */}
                                    ...
                                    {/* resolution notes — shown once resolved */}
...
const styles = {
    ...
    hero: { borderRadius: 14, overflow: "hidden", position: "relative", height: 200, background: "linear-gradient(135deg, var(--color-primary) 0%, var(--color-primary-light) 100%)" },
    heroOverlay: { position: "absolute", inset: 0, backgroundImage: "url('https://images.unsplash.com/photo-1581092918056-0c4c3acd3789?w=1200&q=80')", backgroundSize: "cover", backgroundPosition: "center", opacity: 0.2 },
    ...
    alert:     { display: "flex", alignItems: "center", gap: 10, backgroundColor: "#FEF2F2", border: "1px solid #FECACA", borderRadius: 10, padding: "0.875rem 1.25rem" },
    ...
    guideCard:  { backgroundColor: "var(--color-white)", borderRadius: 14, border: "1px solid var(--color-border)", padding: "1.25rem 1.5rem" },
    guideRow:   { display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginTop: "1rem" },
    guideStep:  { borderRadius: 12, padding: "1.1rem" },
    stepNumber: { width: 28, height: 28, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--color-white)", fontSize: "0.78rem", fontWeight: 700, marginBottom: 8 },
